...
def get_percentile_band(percentile):
...
def test_risk_percentile_at_zero():
    assert get_risk_percentile(0.0, PORTFOLIO) == 0.0

def test_risk_percentile_at_p25():
    assert get_risk_percentile(95.52, PORTFOLIO) == 25.0

def test_risk_percentile_at_median():
    assert get_risk_percentile(117.05, PORTFOLIO) == 50.0

def test_risk_percentile_at_p95():
    assert get_risk_percentile(505.02, PORTFOLIO) == 95.0

def test_risk_percentile_above_p95_capped():
    assert get_risk_percentile(99999.0, PORTFOLIO) == 100.0
...
def test_percentile_band_below_p25():
    assert get_percentile_band(0.0) == 'Below 25th'

def test_percentile_band_between_p25_and_median():
    assert get_percentile_band(25.0) == '25th – 50th'

def test_percentile_band_between_median_and_p75():
    assert get_percentile_band(50.0) == '50th – 75th'

def test_percentile_band_above_p95():
    assert get_percentile_band(100.0) == 'Above 95th'
